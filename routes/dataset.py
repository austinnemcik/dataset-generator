...
def export_dataset(dataset_id: int, session: Session = Depends(get_session)):
    ...
    if not dataset:
        return response_builder(
            success=False, message="No dataset found for this ID", statusCode=404
        )
    ...
    for example in examples:
        formatted = {
            "conversations": [
                ...
                },  # need to save the prompt in the dataset,  then we can pull it out.  also need /delete endpoint
...
def delete_dataset(dataset_id: int, session: Session = Depends(get_session)):
    ...
    session.delete(
        dataset
    )  # we should disable this endpoint by default so it doesn't get exposed since it's not behind auth,  although neither is the rest of it yet. maybe v2
    session.commit()
    return response_builder(
        success=True,
...
async def merge_datasets(body: MergeRequest, session: Session = Depends(get_session)):
    ...
    for id in body.dataset_ids:
        ...
        all_examples.extend(
            examples
        )  # extend adds the whole list,  append just one item
    ...
    try:
        ...
        return response_builder(
            success=False,
            statusCode=400,
            message="An error occurred when calling the naming agent",
        )
